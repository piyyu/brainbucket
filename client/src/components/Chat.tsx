...
export const Chat = ({ messages, setMessages, input, setInput, loading, setLoading }: ChatProps) => {
  ...
  return (
    <div className="h-full w-full flex flex-col overflow-hidden relative bg-background bg-grid">
      {/* Ambient glow */}
      {isEmpty && (
      ...
      {isEmpty && !loading ? (
        ...
        <>
          ...
          {/* Fixed input footer — outside the scroll region so it never drifts */}
          <div className="shrink-0 p-4 z-20 bg-gradient-to-t from-background via-background/95 to-transparent">
